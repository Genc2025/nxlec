...
MANIFEST = ROOT / "data" / "manual_final_qa_wave27_0582_0631.json"
REPORT = ROOT / "FINAL_QA_WAVE27_Q0582_Q0631_MANUAL_10OF10.md"
EXPECTED_IDS = [f"V2-Q{i:04d}" for i in range(582, 632)]
SEMANTIC_DIMS = [
    "source_verified",
    "blueprint_verified",
...
def main() -> None:
    ...
    if not MANIFEST.exists():
        fail("Wave 27 manual QA manifest does not exist")

    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    items = manifest.get("items", [])
    ids = [item.get("question_uid") for item in items]
    if ids != EXPECTED_IDS or len(items) != 50:
        fail("Wave 27 manual manifest scope mismatch")
    for item in items:
        if item.get("manual_disposition") != "PASS":
            fail(f"Manual disposition is not PASS for {item.get('question_uid')}")
        if item.get("all_11_dimensions") is not True:
            fail(f"All 11 dimensions not verified for {item.get('question_uid')}")
        if item.get("second_pass_verified") is not True:
            fail(f"Second-pass QA missing for {item.get('question_uid')}")
        if not str(item.get("source_locator") or "").strip() or not str(item.get("source_url") or "").startswith("https://"):
            fail(f"Source locator evidence missing for {item.get('question_uid')}")
    ...
    for item in items:
        ...
        if q["clinical_qa_status"] != "SOURCE_VERIFIED_2026_WAVE27_ITEM_BY_ITEM":
        ...
        qc = con.execute(
            "SELECT * FROM option_length_qc WHERE question_uid=?", (uid,)
        ).fetchone()
    ...
    passed = con.execute(
        """SELECT COUNT(*) FROM question_final_gate
           WHERE question_uid BETWEEN 'V2-Q0582' AND 'V2-Q0631'
    ...
    if passed != 50 or incomplete != 0:
        con.rollback()
        fail(f"Wave 27 final gate failed: passed={passed}, incomplete={incomplete}")

    con.execute(
        "INSERT OR REPLACE INTO bank_metadata(key,value) VALUES(?,?)",
        ("wave27_q0582_q0631_manual_final_gate", "PASS_50_OF_50_2026_08_14_ITEM_BY_ITEM_STRICT_OPTIONS"),
    )
    con.commit()
    con.close()
    ...
    REPORT.write_text(
        "\n".join([
            "# Manual Final QA Gate — Q0582–Q0631",
            "",
            "- Scope: **50/50** items",
    ...
    print(
        f"Wave27 manual final gate: PASS 50/50; max_ratio={max(ratios):.4f}; "
