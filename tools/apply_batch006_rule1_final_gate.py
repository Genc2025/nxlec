...
REPORT = ROOT / "FINAL_QA_BATCH006_Q0832_Q0881_RULE1_10OF10.md"
...
SECONDARY = {"V2-Q0838","V2-Q0840","V2-Q0844","V2-Q0849","V2-Q0850","V2-Q0851","V2-Q0874"}
SUBSTANTIVE = {"V2-Q0837","V2-Q0839","V2-Q0841","V2-Q0846","V2-Q0852","V2-Q0856","V2-Q0857","V2-Q0859","V2-Q0864","V2-Q0868","V2-Q0874","V2-Q0876"}
CORRECTION_SIGNATURES = {
    "V2-Q0837": ("mg/m²", "regimen"),
    "V2-Q0839": ("narrow", "toxicity"),
    "V2-Q0841": ("renal", "clearance"),
    "V2-Q0846": ("VIS", "manufacturer"),
    "V2-Q0852": ("furosemide", "individual"),
    "V2-Q0856": ("sulfonamide", "cross-reactivity"),
    "V2-Q0857": ("cephalosporin", "cross-reactivity"),
    "V2-Q0859": ("cardiovascular", "cardioselective"),
    "V2-Q0864": ("PPE", "before"),
    "V2-Q0868": ("applicable", "institutional"),
    "V2-Q0874": ("jurisdiction", "statement"),
    "V2-Q0876": ("performance", "privileg"),
}
DIMS = [
...
def load_evidence():
    ...
    if doc.get("batch") != "Q0832-Q0881" or doc.get("legacy_status_evidence") is not False:
        raise SystemExit("Invalid Batch 006 scope or legacy-evidence policy")
    ...
    if set(doc.get("secondary_source_exceptions", [])) != SECONDARY:
        raise SystemExit("Batch006 secondary-source exception set mismatch")
    if set(doc.get("substantive_corrections", [])) != SUBSTANTIVE:
        raise SystemExit("Batch006 substantive-correction set mismatch")
    if doc.get("all_options_semantically_rewritten") is not True:
        raise SystemExit("Batch006 semantic option rewrite evidence missing")
    items = doc.get("items", [])
    by_id = {x["id"]: x for x in items}
    if len(items) != 50 or len(by_id) != 50 or set(by_id) != set(IDS):
        raise SystemExit("Batch006 evidence must contain Q0832-Q0881 exactly once each")
    return by_id
...
def main():
    ...
    con.execute("""CREATE TABLE IF NOT EXISTS rule1_batch006_reaudit_evidence(
      question_uid TEXT PRIMARY KEY,audit_date TEXT NOT NULL,correct_option TEXT NOT NULL,
    ...
    for uid in IDS:
        ...
        if e.get("authority") not in {"P", "S"}:
            ...
            continue
        if (uid in SECONDARY) != (e.get("authority") == "S"):
        ...
        if q["category_id"] != e["category_id"] or q["client_need"] != e["client_need"] or q["difficulty"] != e["difficulty"]:
            ...
            continue
        if (q["source_url"] or "").strip() != e["source_url"].strip():
            failures.append(f"{uid}: verified source URL not integrated")
            continue
        sd = (q["source_detail"] or "").casefold()
        if e["source_locator"].casefold() not in sd or e["source_version"].casefold() not in sd or "2026-08-15" not in sd:
            failures.append(f"{uid}: locator/version/currentness not integrated")
            continue
        if q["clinical_qa_status"] != "SOURCE_VERIFIED_2026_RULE1_BATCH006":
            failures.append(f"{uid}: Batch006 source-verification status missing")
            continue
        ...
        if unique_extreme:
            ...
            continue
        eqc = e.get("option_qc", {})
        if eqc.get("artificial_padding") is not False or eqc.get("semantic_option_rewrite") is not True:
            failures.append(f"{uid}: semantic option-QC evidence incomplete")
            continue
        option_count += 1
        secondary_count += int(uid in SECONDARY)
        ...
        con.execute("""INSERT OR REPLACE INTO rule1_batch006_reaudit_evidence(
        ...
        con.execute(f"""INSERT OR REPLACE INTO question_final_gate(
          ...
          VALUES(?,?,?,?,?,{placeholders},?,?,?)""",
          (uid,now,"OpenAI Rule 1 Batch 006 independent re-audit",e["source_locator"],e["source_version"],
           *([1]*len(DIMS)),json.dumps(metrics,sort_keys=True),None,"FINAL_QA_PASS"))

    if failures:
        con.rollback()
        raise SystemExit("\n".join(failures))
    if correction_count != 12:
        con.rollback(); raise SystemExit(f"Substantive corrections verified {correction_count}/12")
    if secondary_count != 7:
        con.rollback(); raise SystemExit(f"Secondary exceptions verified {secondary_count}/7")
    if option_count != 50:
        con.rollback(); raise SystemExit(f"Option QC verified {option_count}/50")

    con.execute("INSERT OR REPLACE INTO bank_metadata(key,value) VALUES(?,?)",
                ("batch006_q0832_q0881_rule1_final_gate","PASS_50_OF_50_REAL_REAUDIT_SECOND_PASS_11_OF_11_2026_08_15"))
    ...
    if integrity != "ok" or total != 3525 or ready != 0 or not gate or gate[0] != "CLOSED_PENDING_FULL_BANK_CLINICAL_CURRENTNESS_SOURCE_LICENSING_AND_RELEASE_QA":
        con.rollback()
        raise SystemExit(f"Commercial/integrity gate failure: integrity={integrity} total={total} ready={ready} gate={gate}")
    con.commit()

    corrections = ", ".join(sorted(SUBSTANTIVE))
    secondary = ", ".join(sorted(SECONDARY))
    REPORT.write_text(f"""# Rule 1 Batch 006 — Q0832-Q0881 Real Re-audit
...
- All four options semantically rewritten: **50/50**
- Artificial option padding: **NOT USED**
- Substantive corrections: **{corrections}**
- Documented secondary-source exceptions: **{secondary}**
- Commercial release gate: **CLOSED** (`commercial_release_ready=0`)
- Final Batch 006 result: **FINAL_QA_PASS 50/50**

Per-item semantic evidence is persisted in `rule1_batch006_reaudit_evidence`; final-gate state is persisted in `question_final_gate`.
""", encoding="utf-8")
    print(f"RULE1_BATCH006_PASS integrity={integrity} total={total} pass=50/50 evidence11=50/50 manual11=50/50 option_qc={option_count}/50 corrections={correction_count}/12 secondary={secondary_count}/7 max_ratio={max_ratio:.4f} max_dev={max_dev:.4f} ready={ready}")
