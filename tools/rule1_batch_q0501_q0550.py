...
REPORT=Path('RULE1_BATCH_Q0501_Q0550_REPORT.json')
EXPECTED_SOURCE_BLOB='07e335d471ef1b4689406ba41eb98eaa2ca41472'
EXPECTED_CANONICAL_BLOB='67d6657c4e712192a775f3ff524536eba01b5e81'
...
def fingerprint(con):
    rows=con.execute("SELECT * FROM questions WHERE question_uid BETWEEN 'V2-Q0001' AND 'V2-Q0500' ORDER BY question_uid").fetchall()
...
def main():
    uids=[f'V2-Q{i:04d}' for i in range(501,551)]
    ...
    payloads=[]
    for uid in uids:
        try: p=json.loads((APPROVED/f'{uid}.json').read_text(encoding='utf-8'))
    ...
    if head_source!=EXPECTED_SOURCE_BLOB or sh('git','hash-object',str(SOURCE))!=EXPECTED_SOURCE_BLOB: fail(f'source blob moved HEAD={head_source}')
    ...
    if pre!=500 or dup: fail(f'canonical precondition count={pre} dup={dup}')
    if con.execute("SELECT COUNT(*) FROM questions WHERE question_uid BETWEEN 'V2-Q0501' AND 'V2-Q0550'").fetchone()[0]: fail('batch UIDs already present')
    fp_before,fp_count=fingerprint(con)
    if fp_count!=500: fail(f'protected count {fp_count}')
    ...
    for p in payloads:
        ...
        snapshot_exact+=1
    if snapshot_exact!=50: fail('source snapshot validation count failure')
    ...
    if integrity!='ok' or post!=550 or dup2: fail(f'post core failure integrity={integrity} count={post} dup={dup2}')
    ...
    protected=fp_before==fp_after and fp_count==fp_count_after==500
    if not protected: fail('Q0001-Q0500 changed')
    ...
    if next_uid!='V2-Q0551': fail(f'next expected Q0551 got {next_uid}')
    con.close(); src.close()
