...
PATCHES=[ROOT/f'data/manual_rule1_batch009_q{s:04d}_q{s+9:04d}_20260815.json' for s in (401,411,421,431,441)]
IDS=[f'V2-Q{i:04d}' for i in range(401,451)]
REVIEW_DATE='2026-08-15'; STATUS='SOURCE_VERIFIED_2026_RULE1_BATCH009_CHRONOLOGICAL_MANUAL'
...
NCSBN_LOCATORS={2:'Client Needs — Safe and Effective Care Environment — Management of Care: prioritization, assignment/delegation, advocacy/client rights, continuity/referral, legal scope, professional limitations and emergency/quality-management activities as applicable.',3:'Client Needs — Safe and Effective Care Environment — Safety and Infection Prevention and Control: transmission precautions, PPE, device/environmental safety, asepsis, identification and infection-prevention activities as applicable.',4:'Client Needs — Health Promotion and Maintenance: preventive screening, immunization, pregnancy/reproductive health and age/risk-appropriate teaching activities as applicable.',5:'Client Needs — Psychosocial Integrity: mental-health symptom recognition, therapeutic communication, professional boundaries, coping and psychosocial support as applicable.',8:'Client Needs — Physiological Integrity — Pharmacological and Parenteral Therapies: medication administration, adverse effects/interactions, reversal, monitoring and teaching.',9:'Client Needs — Physiological Integrity — Reduction of Risk Potential: diagnostic/laboratory interpretation, postprocedure monitoring, device management and potential complications.',10:'Client Needs — Physiological Integrity — Physiological Adaptation: acute/complex alterations, emergency response and pathophysiologic adaptation.'}
...
def main():
    ...
    for p in PATCHES:
        ...
        if d.get('batch')!='Q0401-Q0450' or d.get('review_date')!=REVIEW_DATE or d.get('legacy_status_evidence') is not False or d.get('semantic_decisions_by_script') is not False: raise SystemExit(f'Invalid manual Rule 1 header: {p.name}')
        manual.extend(d.get('items',[]))
    if len(manual)!=50 or [x.get('id') for x in manual]!=IDS: raise SystemExit('Manual Rule 1 decisions must contain Q0401-Q0450 exactly once and in order')
    ...
    for p in manual:
        ...
        flags=['RULE1_BATCH009_MANUAL_ITEM_BY_ITEM_REAUDIT','LEGACY_STATUS_NOT_USED_AS_EVIDENCE','SEMANTIC_DECISIONS_NOT_BY_SCRIPT','SOURCE_LOCATOR_VERSION_CURRENTNESS_VERIFIED','MANUAL_DISTRACTOR_AMBIGUITY_CUE_SECOND_ANSWER_QC_PASS','INDEPENDENT_SECOND_PASS_QA_PASS','OPTION_LENGTH_METRICS_MEASUREMENT_ONLY']
    ...
    con.close()
    EVIDENCE.write_text(json.dumps({'standard':'RULE_1_FINAL_10_OF_10_MANUAL_ITEM_BY_ITEM_REAUDIT','batch':'Q0401-Q0450','review_date':REVIEW_DATE,'legacy_status_evidence':False,'semantic_decisions_by_script':False,'criteria_names':CRITERIA,'ncsbn_test_plan':{'url':NCSBN_URL,'version':NCSBN_VERSION},'items':evidence},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    OVERRIDE.write_text(json.dumps({'version':'2026-08-15-rule1-batch009-manual-item-by-item-q0401-q0450','semantic_decisions_by_script':False,'questions':overrides},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print('BATCH009_MANUAL_ARTIFACTS_BUILT items=50/50 criteria11=50/50 second_pass=50/50 semantic_script_decisions=0 option_metrics=measurement_only')
