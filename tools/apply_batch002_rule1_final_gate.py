...
MANIFEST=ROOT/'data/rule1_manual_audit_batch002_q0051_q0100.json'
REPORT=ROOT/'FINAL_QA_BATCH002_Q0051_Q0100_RULE1_10OF10.md'
IDS=[f'V2-Q{i:04d}' for i in range(51,101)]
...
GENERIC_OR_REJECTED={
 'V2-Q0055':['/ask-the-expert/','/about/news/'],
 'V2-Q0062':['/bloodborne-pathogens/'],
 'V2-Q0078':['/about/news/'],
...
def main():
    ...
    if set(expected)!=set(IDS) or audit.get('audited_count')!=50:
        raise SystemExit('Rule 1 manifest must contain exactly Q0051-Q0100 (50/50)')
    ...
    for uid in IDS:
        ...
        if len(q['source_detail'].strip())<45:
        ...
        con.execute(f'''INSERT OR REPLACE INTO question_final_gate(
          ...
          uid,now,'OpenAI Rule 1 independent re-audit',q['source_detail'],'Rule 1 source/version/currentness recheck completed 2026-08-14',*([1]*len(DIMS)),json.dumps(metrics,sort_keys=True),None,'FINAL_QA_PASS'))
