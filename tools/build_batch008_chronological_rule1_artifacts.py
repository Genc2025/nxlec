...
PATCHES = [
    ROOT / "data/manual_rule1_batch008_q0351_q0360_20260815.json",
    ROOT / "data/manual_rule1_batch008_q0361_q0370_20260815.json",
    ROOT / "data/manual_rule1_batch008_q0371_q0380_20260815.json",
    ROOT / "data/manual_rule1_batch008_q0381_q0390_20260815.json",
    ROOT / "data/manual_rule1_batch008_q0391_q0400_20260815.json",
]
...
NCSBN_LOCATORS = {
    2: "Client Needs — Safe and Effective Care Environment — Management of Care: continuity/transfer of care, collaboration, legal/regulatory scope, quality improvement, leadership and professional-practice activities as applicable.",
    3: "Client Needs — Safe and Effective Care Environment — Safety and Infection Prevention and Control: patient identification, specimen collection, asepsis, high-alert medication safeguards, fall prevention and communication that prevents safety events as applicable.",
    4: "Client Needs — Health Promotion and Maintenance: preventive care, immunization, pregnancy health promotion, adherence support and age/risk-appropriate teaching activities as applicable.",
    5: "Client Needs — Psychosocial Integrity: mental-health symptom recognition, therapeutic communication, behavioral adaptation and culturally/person-centered psychosocial support as applicable.",
    7: "Client Needs — Physiological Integrity — Basic Care and Comfort: nutrition/food safety, sensory communication, postprocedure comfort, mobility and nonpharmacologic care activities as applicable.",
    8: "Client Needs — Physiological Integrity — Pharmacological and Parenteral Therapies: medication routes, administration, reversal, adverse effects/interactions, withdrawal, monitoring and client teaching.",
    9: "Client Needs — Physiological Integrity — Reduction of Risk Potential: diagnostic probability tools, laboratory/tumor-marker interpretation, ECG/lab monitoring and potential complications.",
    10: "Client Needs — Physiological Integrity — Physiological Adaptation: acute/complex alterations, endocrine/vascular emergencies and pathophysiologic adaptation.",
}
...
def main() -> None:
    if not DB.exists():
        raise SystemExit("Current master DB is required as the content carrier for Batch 008")

    manual: list[dict] = []
    for path in PATCHES:
        doc = load_json(path)
        if (doc.get("batch") != "Q0351-Q0400" or doc.get("review_date") != REVIEW_DATE
    ...
    if [x.get("id") for x in manual] != IDS or len(manual) != 50:
        raise SystemExit("Manual Rule 1 decisions must contain Q0351-Q0400 exactly once and in order")
    ...
    for patch in manual:
        ...
        if category_id not in NCSBN_LOCATORS:
            raise SystemExit(f"{uid}: invalid NCLEX category id {category_id}")
        ...
        finding = patch.get("finding") or (
            ...
            "blueprint/topic/difficulty, source authority/locator/version/currentness and unresolved conflicts were checked. "
            "Any required clinical/editorial/source/blueprint correction was applied before a separate independent second read of the final state."
        )
        source_detail = f"{locator} {version} NCSBN first-check: {ncsbn['locator']} {NCSBN_VERSION}"

        flags = [
            "RULE1_BATCH008_MANUAL_ITEM_BY_ITEM_REAUDIT",
        ...
        evidence_out.append({
            ...
            "second_pass": "PASS",
            "second_pass_method": "Independent fresh second read of the final corrected stem, all four options, key, rationale, source locator/version/currentness, blueprint/difficulty and second-answer/cue risk without using legacy PASS/status as evidence.",
            "final": "FINAL_QA_PASS",
    ...
    OVERRIDE.write_text(json.dumps({
        "version": "2026-08-15-rule1-batch008-manual-item-by-item-q0351-q0400",
        "semantic_decisions_by_script": False,
        "questions": override_out,
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print("BATCH008_MANUAL_ARTIFACTS_BUILT items=50/50 criteria11=50/50 second_pass=50/50 semantic_script_decisions=0 option_metrics=measurement_only")
