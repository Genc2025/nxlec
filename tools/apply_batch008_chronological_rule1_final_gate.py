...
EVIDENCE=ROOT/'data/rule1_batch008_chronological_reaudit_evidence_q0351_q0400.json'
OVERRIDE=ROOT/'data/clinical_overrides_zzz_rule1_batch008_chronological_q0351_q0400_20260815.json'
REPORT=ROOT/'FINAL_QA_BATCH008_Q0351_Q0400_RULE1_10OF10.md'
IDS=[f'V2-Q{i:04d}' for i in range(351,401)]
STATUS='SOURCE_VERIFIED_2026_RULE1_BATCH008_CHRONOLOGICAL_MANUAL'
...
def main():
    ...
    odoc=json.loads(OVERRIDE.read_text(encoding='utf-8'))
    if doc.get('standard')!='RULE_1_FINAL_10_OF_10_MANUAL_ITEM_BY_ITEM_REAUDIT' or doc.get('batch')!='Q0351-Q0400' or doc.get('legacy_status_evidence') is not False or doc.get('semantic_decisions_by_script') is not False or doc.get('criteria_names')!=CRITERIA:
        raise SystemExit('Invalid Batch 008 manual Rule 1 evidence header')
    ev={x['id']:x for x in doc.get('items',[])}
    ov={x['question_uid']:x for x in odoc.get('questions',[])}
    if len(ev)!=50 or set(ev)!=set(IDS) or len(ov)!=50 or set(ov)!=set(IDS):
        raise SystemExit('Batch 008 evidence/override scope mismatch')
    ...
    for uid in IDS:
        ...
        # Persist the already-completed manual semantic blueprint/difficulty decision.
        # This is integration only; no semantic decision is inferred by this script.
        ...
        if not q:
            failures.append(f'{uid}: DB row missing'); continue
        try:
            opts=json.loads(q['item_data_json'])['options']
            key=json.loads(q['correct_answer_json'])['correct_option']
            eopts=json.loads(o['item_data_json'])['options']
            ekey=json.loads(o['correct_answer_json'])['correct_option']
        ...
        if q['clinical_qa_status']!=STATUS:
            failures.append(f'{uid}: final Batch 008 status not integrated'); continue
        flags=set(json.loads(q['editorial_flags_json'] or '[]'))
        needed={'RULE1_BATCH008_MANUAL_ITEM_BY_ITEM_REAUDIT','LEGACY_STATUS_NOT_USED_AS_EVIDENCE','SEMANTIC_DECISIONS_NOT_BY_SCRIPT','SOURCE_LOCATOR_VERSION_CURRENTNESS_VERIFIED','MANUAL_DISTRACTOR_AMBIGUITY_CUE_SECOND_ANSWER_QC_PASS','INDEPENDENT_SECOND_PASS_QA_PASS','OPTION_LENGTH_METRICS_MEASUREMENT_ONLY'}
        if not needed.issubset(flags):
            failures.append(f'{uid}: final manual Rule 1 flags incomplete'); continue
        m=measure(opts,key)
        lv=f"{e['source_locator']} {e['source_version']}"
        con.execute('''INSERT OR REPLACE INTO rule1_batch008_chronological_reaudit_evidence VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(uid,now,key,e['category_id'],e['client_need'],e['difficulty'],e['source_authority'],e['source_url'],e['source_locator'],e['source_version'],e['finding'],json.dumps(n,sort_keys=True),11,'PASS','FINAL_QA_PASS',json.dumps(m,sort_keys=True)))
    ...
    final=con.execute("SELECT COUNT(*) FROM question_final_gate WHERE question_uid BETWEEN 'V2-Q0351' AND 'V2-Q0400' AND final_status='FINAL_QA_PASS'").fetchone()[0]
    manual=con.execute("SELECT COUNT(*) FROM rule1_manual_audit WHERE question_uid BETWEEN 'V2-Q0351' AND 'V2-Q0400' AND criteria_passed_count=11 AND second_pass='PASS' AND final_disposition='FINAL_QA_PASS'").fetchone()[0]
    evidence=con.execute("SELECT COUNT(*) FROM rule1_batch008_chronological_reaudit_evidence WHERE criteria_passed_count=11 AND second_pass='PASS' AND final_disposition='FINAL_QA_PASS'").fetchone()[0]
    bad=con.execute("SELECT COUNT(*) FROM question_final_gate WHERE question_uid BETWEEN 'V2-Q0351' AND 'V2-Q0400' AND (source_verified<>1 OR blueprint_verified<>1 OR question_quality_verified<>1 OR correct_answer_verified<>1 OR distractors_verified<>1 OR explanation_verified<>1 OR currentness_verified<>1 OR independent_qa_passed<>1 OR no_unresolved_conflict<>1)").fetchone()[0]
    if integrity!='ok' or total!=3525 or ready!=0 or not gate or gate[0]!=GATE or (final,manual,evidence,bad)!=(50,50,50,0):
        con.rollback(); raise SystemExit(f'Batch008 integrated gate failed integrity={integrity} total={total} ready={ready} final={final} manual={manual} evidence={evidence} bad={bad} gate={gate}')
    con.commit(); con.close()

    REPORT.write_text('\n'.join([
        '# Rule 1 Chronological Batch 008 — Q0351-Q0400','',
        '- Scope: **50/50**',
        '- Legacy PASS/status used as clinical quality evidence: **NO**',
        '- Semantic/clinical decisions made by scripts: **NO**',
        '- Fresh item-by-item review of stem + all four options + key + rationale: **50/50**',
        '- Rule 1 criteria: **11/11 for 50/50**',
        '- Correct answers directly source-verified: **50/50**',
        '- Source authority + URL + exact locator + version/currentness: **50/50**',
        ...
        '- Real clinical/editorial/source/blueprint corrections integrated where required: **YES**',
        '- Independent second pass on final corrected state: **50/50**',
        '- Unresolved conflicts: **0**',
        '- Option-length metrics: **measurement only; not a semantic PASS rule**',
        '- SQLite integrity: **ok**',
        ...
        '## Final disposition','**FINAL_QA_PASS — 50/50 for Batch 008 Q0351-Q0400.**'
    ])+'\n',encoding='utf-8')
    print('BATCH008_MANUAL_FINAL_GATE PASS=50/50 criteria11=50/50 sources=50/50 second_pass=50/50 unresolved=0 bad=0 integrity=ok')
