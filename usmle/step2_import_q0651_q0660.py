...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0660.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0660.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='8977d8ae62f88b90b5c020b19dd729a46cc3009d'
AUDIT_ID='STEP2-FINAL-Q0001-Q0660-20260828'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0651_Q0660|2026-08-28'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':84,
    'Blood, Lymphoreticular and Immune Systems':71,
    'Behavioral Health, Nervous Systems and Special Senses':78,
    'Musculoskeletal, Skin and Subcutaneous Tissue':65,
    'Cardiovascular System':58,
    'Gastrointestinal System':52,
    'Reproductive and Endocrine Systems':91,
    'Multisystem Processes and Disorders':65,
    'Biostatistics, Epidemiology and Population Health':33,
    'Social Sciences: Communication and Interpersonal Skills':49,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':431,
    'Patient Care: Diagnosis, including history and physical examination':147,
    'Practice-Based Learning and Improvement':33,
    'Communication and Interpersonal Skills':49,
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {651+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260828T133000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0651-Q0660-20260828',
...
def main():
    ...
    if len(old_items)!=650 or len(old_reviews)!=650:
        raise SystemExit(f'expected 650 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,651)):
    ...
    for n in range(651,661):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=650:
    ...
    seq=''.join(schedule[n] for n in range(651,661))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,660,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=660 or len(revs)!=660:
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=660:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':660,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':660,
        'new_block':{'range':'Q0651-Q0660','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':660,'new_block_reread_verified_count':10,
        'q0651_q0660_present_exactly_once':True,'contiguous_q0001_q0660':True,
