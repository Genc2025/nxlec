...
EXPECTED_SYSTEM_COUNTS = {
 "Human Development":10,"Blood, Lymphoreticular and Immune Systems":55,
 "Behavioral Health, Nervous Systems and Special Senses":60,
 "Musculoskeletal, Skin and Subcutaneous Tissue":50,"Cardiovascular System":45,
 "Respiratory and Renal/Urinary Systems":65,"Gastrointestinal System":40,
 "Reproductive and Endocrine Systems":65,"Multisystem Processes and Disorders":50,
...
EXPECTED_KEYS = {k:100 for k in "ABCDE"}
BATCH_TIME="2026-08-27T03:40:00Z"
SPEC={
...
def build_pair(x):
 n=x["num"]; cid=f"S1-DIRECT-{n:04d}-202608270340Z"; key=x["key"]
...
def load_specs(spec_dir):
 ...
 if len(xs)!=100 or [x["num"] for x in xs]!=list(range(401,501)):
  raise SystemExit(f"spec set must contain exactly Q0401-Q0500; got {len(xs)} items")
 keys="ABCDE"*20
...
def main(spec_dir):
 ...
 if before!=400:raise SystemExit(f"expected authoritative count 400, found {before}")
 ...
 if after!=500 or after-before!=100:raise SystemExit(f"counter mismatch {before}->{after}")
 for p in pairs:
