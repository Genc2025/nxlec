...
FREEZE_DATE=os.environ.get('FREEZE_DATE','2026-09-10')
...
def fetch(url):
 ...
 if 'pubmed.ncbi.nlm.nih.gov' in host:
  ...
  if not m:
   raise RuntimeError('invalid PubMed URL')
  url='https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id='+m.group(1)+'&retmode=xml'
 req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 USMLE-Freeze/2.3'})
...
def resolve_part(nt,part):
 ...
 for a in anchors:
  ...
  while True:
   ...
   start=i+1
 if not candidates:
  raise RuntimeError(f'cited locator component not resolved: {part}')
...
def cited_material(text,locator):
 ...
 if not parts:
  raise RuntimeError('empty cited locator')
...
def main():
 ...
 assert [x['num'] for x in b['items']]==list(range(START,END+1))
 strip_ncjmm(b)
 cache={}
 for x in b['items']:
  for s in x.get('sources',[]):
   ...
   if ck not in cache:
    raw=fetch(url)
    material,nparts=cited_material(clean(raw),loc)
    cache[ck]=(hashlib.sha256(raw).hexdigest(),hashlib.sha256(material.encode()).hexdigest(),nparts)
   psha,csha,nparts=cache[ck]
   s['source_page_sha256']=psha
   s['cited_section_sha256']=csha
   s['cited_locator_component_count']=nparts
   if 'mechanism of action' in nloc(loc):
    s['mechanism_section_sha256']=csha
   s['hash_status']=f'FROZEN_FROM_LIVE_REFETCH_ALL_DECLARED_LOCATOR_COMPONENTS_{FREEZE_DATE}'
 ti=b.setdefault('technical_integrity',{})
 ti['source_hashes_complete']=True
 ti['required_before_freeze']=f'COMPLETE_{FREEZE_DATE}: source_page_sha256 + cited_section_sha256 bind every declared locator component'
 ti['independent_audit_complete']=False
 for x in b['items']:
  aq=x.get('author_qa',{})
  if 'technical_hash_gate' in aq:
   aq['technical_hash_gate']=f'PASS — live refetch and hashes for every declared locator component completed {FREEZE_DATE}.'
