...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0800.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0800.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='e15831a97d9c5370e3a921432e1ff6431003c4d3'
AUDIT_ID='STEP2-FINAL-Q0001-Q0800-20260830'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0791_Q0800|2026-08-30'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':101,
    'Blood, Lymphoreticular and Immune Systems':86,
    'Behavioral Health, Nervous Systems and Special Senses':93,
    'Musculoskeletal, Skin and Subcutaneous Tissue':79,
    'Cardiovascular System':72,
    'Gastrointestinal System':64,
    'Reproductive and Endocrine Systems':109,
    'Multisystem Processes and Disorders':79,
    'Biostatistics, Epidemiology and Population Health':42,
    'Social Sciences: Communication and Interpersonal Skills':59,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':521,
    'Patient Care: Diagnosis, including history and physical examination':176,
    'Practice-Based Learning and Improvement':43,
    'Communication and Interpersonal Skills':60,
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {791+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260830T200000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0791-Q0800-20260830',
...
def main():
    ...
    if len(old_items)!=790 or len(old_reviews)!=790:
        raise SystemExit(f'expected 790 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,791)):
    ...
    for n in range(791,801):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=790:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,800,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=800 or len(revs)!=800:
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=800:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':800,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':800,
        'new_block':{'range':'Q0791-Q0800','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':800,'new_block_reread_verified_count':10,
        'q0791_q0800_present_exactly_once':True,'contiguous_q0001_q0800':True,
