...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0630.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0630.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0621_Q0630|2026-08-27'
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {621+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f"S1-DIRECT-{x['num']:04d}-20260827T160000Z"
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,
        'step2_audit_id':'STEP2-FINAL-Q0621-Q0630-20260827',
...
def main():
    ...
    if len(old_items)!=620 or len(old_reviews)!=620:
        raise SystemExit(f'expected 620 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_nums!=set(range(1,621)):
        raise SystemExit('pre canonical Q0001-Q0620 coverage failure')
    ...
    for n in range(621,631):
        ...
        finals[n]=c
        reviews[n]=r
    # Technical duplicate gates against authoritative 620 and within approved batch.
    ...
    seq=''.join(schedule[n] for n in range(621,631))
    ...
    try:
        ...
        con.execute(
            ...
            ('STEP2-FINAL-Q0001-Q0630-20260827',630,key_hash,aggregate_review_hash,FINAL_AT)
        )
        con.commit()
    except:
        con.rollback()
    ...
    if item_count!=630 or review_count!=630:
    ...
    if nums!=set(range(1,631)):
        raise SystemExit('post Q0001-Q0630 coverage failure')
    reread=0
    new_reread=0
    for cid,pj,ps,ash in con.execute("SELECT candidate_id,payload_json,payload_sha256,audit_sha256 FROM step2_final_items WHERE final_status='FINAL_10_10_PASS' ORDER BY candidate_id"):
        ...
        if 621<=n<=630:
            ...
            new_reread+=1
        reread+=1
    if reread!=630 or new_reread!=10:
    ...
    expected_blueprint={
        ...
        'Respiratory and Renal/Urinary Systems':82,
        'Blood, Lymphoreticular and Immune Systems':69,
        'Behavioral Health, Nervous Systems and Special Senses':75,
        'Musculoskeletal, Skin and Subcutaneous Tissue':63,
        'Cardiovascular System':57,
        'Gastrointestinal System':50,
        'Reproductive and Endocrine Systems':83,
    ...
    expected_competencies={
        'Medical Knowledge: Applying Foundational Science Concepts':416,
