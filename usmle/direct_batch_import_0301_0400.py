...
EXPECTED_SYSTEM_COUNTS = {
 "Human Development":8,"Blood, Lymphoreticular and Immune Systems":44,
 "Behavioral Health, Nervous Systems and Special Senses":48,
 "Musculoskeletal, Skin and Subcutaneous Tissue":40,"Cardiovascular System":36,
 "Respiratory and Renal/Urinary Systems":52,"Gastrointestinal System":32,
 "Reproductive and Endocrine Systems":52,"Multisystem Processes and Disorders":40,
 "Biostatistics, Epidemiology and Population Health":20,
 "Social Sciences: Communication and Interpersonal Skills":28}
EXPECTED_COMPETENCY_COUNTS = {
 "Medical Knowledge: Applying Foundational Science Concepts":260,
 "Patient Care: Diagnosis, including history and physical examination":92,
 "Practice-Based Learning and Improvement":20,
 "Communication and Interpersonal Skills":28}
EXPECTED_KEYS = {k:80 for k in "ABCDE"}
BATCH_TIME="2026-08-27T03:20:00Z"
SPEC={
...
def build_pair(x):
 n=x["num"]; cid=f"S1-DIRECT-{n:04d}-202608270320Z"; key=x["key"]
...
def load_specs(spec_dir):
 ...
 if len(xs)!=100 or [x["num"] for x in xs]!=list(range(301,401)):
  raise SystemExit(f"spec set must contain exactly Q0301-Q0400; got {len(xs)} items")
 keys="ABCDE"*20
...
def main(spec_dir):
 ...
 if before!=300:raise SystemExit(f"expected authoritative count 300, found {before}")
 ...
 if after!=400 or after-before!=100:raise SystemExit(f"counter mismatch {before}->{after}")
 for p in pairs:
