...
ROOT=Path(__file__).resolve().parent; BATCH=ROOT/'batch_specs_1201_1300'/'10_q1271_q1280_author_20260906.json'; DB=ROOT/'data'/'usmle-step1.db'
...
ANCHORS={1271:['kcnq2','kv7.2'],1272:['slc12a5','kcc2'],1273:['dnm1','dynamin-1'],1274:['tubb4a'],1275:['chrna4'],1276:['fermt3','kindlin-3'],1277:['lrba'],1278:['unc13d','munc13-4'],1279:['ap3b1'],1280:['kcnn4','gardos channel']}
...
def main():
    ...
    assert b['production_count_before']==b['production_count_after']==1270 and b['status']=='AUTHOR_ZERO_TRUST_PASS_PENDING_CANONICAL_DUPLICATE_AND_FINAL_QA' and b['specification_version']==CURRENT
    assert [x['num'] for x in items]==list(range(1271,1281)) and b['answer_key_sequence']=='CEADBADCBE' and b['answer_key_distribution']=={'A':2,'B':2,'C':2,'D':2,'E':2}
    assert Counter(x['blueprint']['primary_system'] for x in items)==Counter({NEURO:5,BLOOD:5});assert Counter(x['blueprint']['primary_competency'] for x in items)==Counter({DX:5,MK:5})

    raw=html.unescape(fetch_retry(USMLE).decode('utf-8','ignore')); page=' '.join(re.sub(r'<[^>]+>',' ',raw).split())
    for label in [NEURO,BLOOD,MK,DX,'Pathology','Physiology','Immunology','Genetics','Histology & Cell Biology']:
        assert label in page,('official current label missing',label)
    assert re.search(r'Blood\s*&\s*Lymphoreticular/Immune Systems\s*\|?\s*9-13',page)
    assert re.search(r'Behavioral Health\s*&\s*Nervous Systems/Special Senses\s*\|?\s*10.?14',page)
    ...
    for x in items:
        ...
        assert x['specification_version']==CURRENT and bp['primary_system'] in {NEURO,BLOOD} and bp['primary_competency'] in {DX,MK} and bp['official_outline_path']==[bp['primary_system']] and bp['internal_content_path']
    ...
    con=sqlite3.connect(DB.resolve().as_uri()+'?mode=ro&immutable=1',uri=True);assert con.execute('pragma integrity_check').fetchone()[0]=='ok';rows=con.execute("select candidate_id,payload_json from step2_final_items where final_status='FINAL_10_10_PASS'").fetchall();con.close();assert len(rows)==1270
