...
SPEC = ROOT / 'batch_specs_1101_1200' / '11_q1186_q1190.json'
AUDIT_DIR = ROOT / 'audit'
PREFLIGHT = AUDIT_DIR / 'PREFLIGHT_Q1186_Q1190.json'
FINAL_AUDIT = AUDIT_DIR / 'STEP2_FINAL_10_10_Q0001_Q1190.json'
FINAL_STATE = ROOT / 'state' / 'step2_final_q0001_q1190.json'
FINAL_AT = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')

EXPECTED_PRE_DB_BLOB = '05dc982aa4a1c35cccfc233f80cd7826fc22bec9'
EXPECTED_SPEC_BLOB = '605330a6457e10acae9855fb8556a304d221cf32'
AUDIT_ID = 'STEP2-FINAL-Q0001-Q1190-20260904'
NEW_RANGE = range(1186,1191)
KEY_SCHEDULE = {1186:'D',1187:'A',1188:'E',1189:'C',1190:'B'}

EXPECTED_BLUEPRINT = {
    "Human Development":33,
    "Respiratory and Renal/Urinary Systems":149,
    "Blood, Lymphoreticular and Immune Systems":127,
    "Behavioral Health, Nervous Systems and Special Senses":135,
    "Musculoskeletal, Skin and Subcutaneous Tissue":115,
    "Cardiovascular System":105,
    "Gastrointestinal System":99,
    "Reproductive and Endocrine Systems":156,
    "Multisystem Processes and Disorders":127,
    "Biostatistics, Epidemiology and Population Health":65,
    "Social Sciences: Communication and Interpersonal Skills":79,
}
EXPECTED_COMPETENCIES = {
    "Medical Knowledge: Applying Foundational Science Concepts":806,
    "Patient Care: Diagnosis, including history and physical examination":238,
    "Practice-Based Learning and Improvement":66,
    "Communication and Interpersonal Skills":80,
}
...
def load_preflight():
    ...
    if p.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(p.get('authoritative_final_count',-1))!=1185:
...
def load_audits():
    ...
    for n in NEW_RANGE:
        ...
        if a.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(a.get('authoritative_db_final_count',-1))!=1185:
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260904T061000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,
        'step2_audit_id':'STEP2-FINAL-Q1186-Q1190-20260904',
...
def main():
    ...
    if len(old_items)!=1185 or len(old_reviews)!=1185:
        raise SystemExit(f'expected 1185 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,1186)):
    ...
    for n in NEW_RANGE:
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=1185:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',
                    (AUDIT_ID,1190,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback()
        raise
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=1190:
    ...
    result={
        ...
        'q1186_q1190_present_exactly_once':True,
        'contiguous_q0001_q1190':True,
