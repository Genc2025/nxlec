...
SPEC = ROOT / 'batch_specs_1101_1200' / '08_q1171_q1175.json'
AUDIT_DIR = ROOT / 'audit'
PREFLIGHT = AUDIT_DIR / 'PREFLIGHT_Q1171_Q1175.json'
FINAL_AUDIT = AUDIT_DIR / 'STEP2_FINAL_10_10_Q0001_Q1175.json'
FINAL_STATE = ROOT / 'state' / 'step2_final_q0001_q1175.json'
...
EXPECTED_BLUEPRINT = {
    ...
    "Respiratory and Renal/Urinary Systems":146,
    "Blood, Lymphoreticular and Immune Systems":127,
    "Behavioral Health, Nervous Systems and Special Senses":132,
    "Musculoskeletal, Skin and Subcutaneous Tissue":112,
    "Cardiovascular System":105,
    "Gastrointestinal System":99,
    "Reproductive and Endocrine Systems":152,
...
EXPECTED_COMPETENCIES = {
    "Medical Knowledge: Applying Foundational Science Concepts":796,
    "Patient Care: Diagnosis, including history and physical examination":235,
...
def load_preflight():
    ...
    if p.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(p.get('authoritative_final_count',-1))!=1170:
...
def load_audits():
    ...
    for n in NEW_RANGE:
        ...
        if a.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(a.get('authoritative_db_final_count',-1))!=1170:
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260903T230000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,
        'step2_audit_id':'STEP2-FINAL-Q1171-Q1175-20260903',
...
def main():
    ...
    if len(old_items)!=1170 or len(old_reviews)!=1170:
        raise SystemExit(f'expected 1170 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,1171)):
    ...
    for n in NEW_RANGE:
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=1170:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',
                    (AUDIT_ID,1175,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback()
        raise
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=1175:
    ...
    result={
        ...
        'q1171_q1175_present_exactly_once':True,
