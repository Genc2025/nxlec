...
SPEC_DIR=ROOT/'batch_specs_0901_1000'
AUDIT_DIR=ROOT/'audit'
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0990.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0990.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='deb836a6f997e32b3139c4014707ff7605e47bff'
AUDIT_ID='STEP2-FINAL-Q0001-Q0990-20260901'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0981_Q0990|2026-09-01'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':127,
    'Blood, Lymphoreticular and Immune Systems':108,
    'Behavioral Health, Nervous Systems and Special Senses':115,
    'Musculoskeletal, Skin and Subcutaneous Tissue':93,
    'Cardiovascular System':90,
    'Gastrointestinal System':81,
    'Reproductive and Endocrine Systems':131,
    'Multisystem Processes and Disorders':106,
    'Biostatistics, Epidemiology and Population Health':52,
    'Social Sciences: Communication and Interpersonal Skills':67,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':670,
    'Patient Care: Diagnosis, including history and physical examination':199,
    'Practice-Based Learning and Improvement':53,
...
EXPECTED_NEW_DISCIPLINES={
    'Pathology':4,'Physiology':6,'Nutrition':0,'Gross Anatomy & Embryology':0,'Microbiology':0,
    'Pharmacology':0,'Behavioral Sciences':1,'Biochemistry':6,'Histology & Cell Biology':1,
    'Immunology':0,'Genetics':5
}
...
def load_audits():
    ...
    for n in range(981,991):
        ...
        overlap_status=a.get('canonical_main_construct_overlap')
        allowed_overlap={'PASS_NONE','PASS_ADJACENT_DISTINCT','PASS_MENTION_ONLY'}
        if overlap_status not in allowed_overlap:
            raise SystemExit(f'Q{n:04d}: canonical main-construct overlap gate invalid: {overlap_status!r}')
        if overlap_status!='PASS_NONE' and not str(a.get('canonical_probe_note','')).strip():
            raise SystemExit(f'Q{n:04d}: audited overlap classification lacks probe note')
        if a.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(a.get('authoritative_db_final_count',-1))!=980:
            raise SystemExit(f'Q{n:04d}: audit bound to wrong authoritative DB state')
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {981+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260901T210000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0981-Q0990-20260901',
...
def main():
    ...
    current_spec_blob=subprocess.check_output(['git','-C',str(REPO),'hash-object','usmle/batch_specs_0901_1000/09_q0981_q0990.json'],text=True).strip()
    for n,(a,_) in audits.items():
        if a.get('exact_spec_sha')!=current_spec_blob:
            raise SystemExit(f'Q{n:04d}: audit spec binding mismatch {a.get("exact_spec_sha")} != {current_spec_blob}')
    ...
    if len(old_items)!=980 or len(old_reviews)!=980:
        raise SystemExit(f'expected 980 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,991)):
    ...
    for n in range(981,991):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=980:
    ...
    seq=''.join(schedule[n] for n in range(981,991))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,990,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=990 or len(revs)!=990:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,981)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(981,991)):
        raise SystemExit('Q0981-Q0990 exact-once failure')
    ...
    for cid,pj,ps,ash in items:
        ...
        if 981<=n<=990:
            ...
            if ca.get('canonical_main_construct_overlap') not in {'PASS_NONE','PASS_ADJACENT_DISTINCT','PASS_MENTION_ONLY'}:
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=990:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':990,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':990,
        'new_block':{'range':'Q0981-Q0990','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'official_discipline_reference':{
            ...
            'note':'Q0981-Q0990 use explicit official Table 3 discipline tags. Legacy FINAL payloads remain immutable; cumulative Table 3 percentages are not fabricated from heterogeneous legacy labels.'
        },
        ...
        'payload_review_consistency':'PASS','reread_verified_count':990,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q0981_q0990_present_exactly_once':True,'contiguous_q0001_q0990':True,
