...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0770.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0770.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='e570dcd3251c5d26b37944e64536590e1db08e1b'
AUDIT_ID='STEP2-FINAL-Q0001-Q0770-20260830'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0761_Q0770|2026-08-30'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':98,
    'Blood, Lymphoreticular and Immune Systems':83,
    'Behavioral Health, Nervous Systems and Special Senses':90,
    'Musculoskeletal, Skin and Subcutaneous Tissue':76,
    'Cardiovascular System':69,
    'Gastrointestinal System':61,
    'Reproductive and Endocrine Systems':106,
    'Multisystem Processes and Disorders':76,
    'Biostatistics, Epidemiology and Population Health':39,
    'Social Sciences: Communication and Interpersonal Skills':57,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':499,
    'Patient Care: Diagnosis, including history and physical examination':173,
    'Practice-Based Learning and Improvement':40,
    'Communication and Interpersonal Skills':58,
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {761+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260830T170000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0761-Q0770-20260830',
...
def main():
    ...
    if len(old_items)!=760 or len(old_reviews)!=760:
        raise SystemExit(f'expected 760 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,761)):
    ...
    for n in range(761,771):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=760:
    ...
    seq=''.join(schedule[n] for n in range(761,771))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,770,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=770 or len(revs)!=770:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,771)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(761,771)):
        raise SystemExit('Q0761-Q0770 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=770:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':770,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':770,
        'new_block':{'range':'Q0761-Q0770','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':770,'new_block_reread_verified_count':10,
        'q0761_q0770_present_exactly_once':True,'contiguous_q0001_q0770':True,
