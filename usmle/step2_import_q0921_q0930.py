...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0930.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0930.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='d179b89f813148d689337e43c313b9af8d8eca31'
AUDIT_ID='STEP2-FINAL-Q0001-Q0930-20260901'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0921_Q0930|2026-09-01'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':120,
    ...
    'Reproductive and Endocrine Systems':124,
    'Multisystem Processes and Disorders':98,
    'Biostatistics, Epidemiology and Population Health':50,
    'Social Sciences: Communication and Interpersonal Skills':66,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':623,
...
EXPECTED_NEW_DISCIPLINES={
    'Pathology':4,'Physiology':5,'Nutrition':0,'Gross Anatomy & Embryology':0,'Microbiology':6,
    'Pharmacology':8,'Behavioral Sciences':0,'Biochemistry':2,'Histology & Cell Biology':0,
    'Immunology':1,'Genetics':0
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {921+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260901T063000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0921-Q0930-20260831',
...
def main():
    ...
    if len(old_items)!=920 or len(old_reviews)!=920:
        raise SystemExit(f'expected 920 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,921)):
    ...
    for n in range(921,931):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=920:
    ...
    seq=''.join(schedule[n] for n in range(921,931))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,930,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=930 or len(revs)!=930:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,931)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(921,931)):
        raise SystemExit('Q0921-Q0930 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=930:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':930,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':930,
        'new_block':{'range':'Q0921-Q0930','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'official_discipline_reference':{
            ...
            'note':'Q0921-Q0930 use explicit official Table 3 discipline tags. Legacy FINAL payloads remain immutable; cumulative Table 3 percentages are not fabricated from heterogeneous legacy labels.'
        },
        ...
        'payload_review_consistency':'PASS','reread_verified_count':930,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q0921_q0930_present_exactly_once':True,'contiguous_q0001_q0930':True,
