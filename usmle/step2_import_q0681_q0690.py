...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0690.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0690.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='6713c18b8787360a1ad0841a4bfbb7f47cbad6eb'
AUDIT_ID='STEP2-FINAL-Q0001-Q0690-20260829'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0681_Q0690|2026-08-29'
SYSTEM_ALIASES={
...
def build_payload(template,x,key,audit,audit_path):
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0681-Q0690-20260829',
...
def main():
    ...
    if len(old_items)!=680 or len(old_reviews)!=680:
        raise SystemExit(f'expected 680 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    for n in range(681,691):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=680:
    ...
    seq=''.join(schedule[n] for n in range(681,691))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,690,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=690 or len(revs)!=690:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,691)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(681,691)):
        raise SystemExit('Q0681-Q0690 exact-once failure')
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':690,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':690,
        'new_block':{'range':'Q0681-Q0690','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':690,'new_block_reread_verified_count':10,
        'q0681_q0690_present_exactly_once':True,'contiguous_q0001_q0690':True,
