...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q1040.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q1040.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='d43c2cc23ad4ecbdcf622cd157c6e601435319ee'
AUDIT_ID='STEP2-FINAL-Q0001-Q1040-20260902'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q1031_Q1040|2026-09-02'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    "Behavioral Health, Nervous Systems and Special Senses":118,
    "Musculoskeletal, Skin and Subcutaneous Tissue":97,
    "Cardiovascular System":93,
    "Gastrointestinal System":87,
    "Reproductive and Endocrine Systems":136,
    "Multisystem Processes and Disorders":111,
    "Biostatistics, Epidemiology and Population Health":57,
    "Social Sciences: Communication and Interpersonal Skills":70,
}
EXPECTED_COMPETENCIES={
    "Medical Knowledge: Applying Foundational Science Concepts":703,
    "Patient Care: Diagnosis, including history and physical examination":208,
    "Practice-Based Learning and Improvement":58,
...
EXPECTED_NEW_DISCIPLINES={
    "Pathology":1,"Physiology":7,"Nutrition":0,"Gross Anatomy & Embryology":2,"Microbiology":0,"Pharmacology":0,"Behavioral Sciences":2,"Biochemistry":3,"Histology & Cell Biology":1,"Immunology":0,"Genetics":2
}
...
def load_audits():
    ...
    for n in range(1031,1041):
        ...
        if a.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(a.get('authoritative_db_final_count',-1))!=1030:
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {1031+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260902T081500Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q1031-Q1040-20260902',
...
def main():
    ...
    current_spec_blob=subprocess.check_output(['git','-C',str(REPO),'hash-object','usmle/batch_specs_1001_1100/04_q1031_q1040.json'],text=True).strip()
    ...
    if len(old_items)!=1030 or len(old_reviews)!=1030:
        raise SystemExit(f'expected 1030 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,1031)):
    ...
    for n in range(1031,1041):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=1030:
    ...
    seq=''.join(schedule[n] for n in range(1031,1041))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,1040,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=1040 or len(revs)!=1040:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,1041)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(1031,1041)):
        raise SystemExit('Q1031-Q1040 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=1040:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':1040,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':1040,
        'new_block':{'range':'Q1031-Q1040','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'official_discipline_reference':{
            ...
            'note':'Q1031-Q1040 use explicit official Table 3 discipline tags. Legacy FINAL payloads remain immutable; cumulative Table 3 percentages are not fabricated from heterogeneous legacy labels.'
        },
        ...
        'payload_review_consistency':'PASS','reread_verified_count':1040,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q1031_q1040_present_exactly_once':True,'contiguous_q0001_q1040':True,
