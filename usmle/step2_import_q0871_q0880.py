...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0880.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0880.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='28e8757d689674640e0c08cf5fa636a9802d2949'
AUDIT_ID='STEP2-FINAL-Q0001-Q0880-20260831'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0871_Q0880|2026-08-31'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':108,
    'Blood, Lymphoreticular and Immune Systems':95,
    'Behavioral Health, Nervous Systems and Special Senses':101,
    'Musculoskeletal, Skin and Subcutaneous Tissue':87,
    'Cardiovascular System':80,
    'Gastrointestinal System':71,
    'Reproductive and Endocrine Systems':117,
...
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':576,
    'Patient Care: Diagnosis, including history and physical examination':186,
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260831T050000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0871-Q0880-20260831',
...
def main():
    ...
    if len(old_items)!=870 or len(old_reviews)!=870:
        raise SystemExit(f'expected 870 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,871)):
    ...
    for n in range(871,881):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=870:
    ...
    seq=''.join(schedule[n] for n in range(871,881))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,880,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=880 or len(revs)!=880:
    ...
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(871,881)):
        raise SystemExit('Q0871-Q0880 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=880:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':880,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':880,
        'new_block':{'range':'Q0871-Q0880','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':880,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q0871_q0880_present_exactly_once':True,'contiguous_q0001_q0880':True,
