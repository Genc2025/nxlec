...
STATE_PRE=ROOT/'state'/'step2_final_q0001_q1260.json'
STATE_POST=ROOT/'state'/'step2_final_q0001_q1270.json'
AUD=ROOT/'audit'
MANIFEST=AUD/'Q1261_Q1270_FINAL_QA_PASS.json'
FINAL_AUDIT=AUD/'STEP2_FINAL_10_10_Q0001_Q1270.json'
BATCH=ROOT/'batch_specs_1201_1300'/'09_q1261_q1270_author_20260906.json'
NEW_RANGE=range(1261,1271)
EXPECTED_KEYS={1261:'B',1262:'E',1263:'C',1264:'A',1265:'D',1266:'C',1267:'A',1268:'D',1269:'E',1270:'B'}
EXPECTED_PRE_COUNT=1260; EXPECTED_POST_COUNT=1270
EXPECTED_PRE_DB_BLOB='2afb11e1d3490cedc757ecfbe7e4515c6bbd527c'
EXPECTED_BATCH_BLOB='25d7f71b6aa7866e5c5a8f0da7c7c9bc28e309ef'
EXPECTED_MANIFEST_BLOB='61e85d42cb173ee7ee5fc6f413161d7d1a3309e7'
AUDIT_ID='STEP2-FINAL-Q0001-Q1270-20260906'; CID_SUFFIX='20260906T101000Z'
...
def load_candidates():
    ...
    b=json.loads(BATCH.read_text()); docs={int(x['num']):x for x in b.get('items',[])}
    ...
    seq=''.join(docs[n]['item']['intended_key'] for n in NEW_RANGE)
    if seq!='BECADCADEB' or Counter(seq)!=Counter({'A':2,'B':2,'C':2,'D':2,'E':2}): raise SystemExit('candidate key failure')
    if Counter(d['blueprint']['primary_system'] for d in docs.values())!=Counter({GI:5,CV:5}): raise SystemExit('candidate system distribution failure')
    ...
    if 'ncjmm' in BATCH.read_text().casefold(): raise SystemExit('NCJMM contamination')
    return b,docs

def source_normalize(s,idx):
    u=str(s.get('url','')).strip(); loc=str(s.get('section_locator','')).strip()
    if not authority_ok(u) or not loc: raise SystemExit('source authority/locator failure '+u)
...
def load_manifest_and_audits(docs):
    ...
    if m.get('status')!='FINAL_QA_PASS' or m.get('final_qa_verdict')!='FINAL_QA_PASS_NO_MATERIAL_DEFECT' or m.get('production_import_ready') is not True or m.get('production_db_modified') is not False: raise SystemExit('FINAL QA status failure')
    ...
    if m.get('item_count')!=10 or m.get('item_range')!='Q1261-Q1270' or m.get('fresh_rerun_from_zero') is not True: raise SystemExit('FINAL QA range/freshness failure')
    if m.get('answer_key_sequence')!='BECADCADEB' or m.get('answer_key_distribution')!={'A':2,'B':2,'C':2,'D':2,'E':2}: raise SystemExit('FINAL QA answer-position failure')
    if m.get('system_distribution')!={GI:5,CV:5} or m.get('competency_distribution')!={DX:5,MK:5}: raise SystemExit('FINAL QA distribution failure')
    for k in ('all_items_final_10_10_pass','production_import_ready'):
        if m.get(k) is not True: raise SystemExit('FINAL QA boolean gate '+k)
    if m.get('second_answer_attack_all')!='PASS_NONE' or m.get('canonical_duplicate_gate')!='PASS' or m.get('within_batch_collision_gate')!='PASS' or m.get('source_locator_currentness_gate')!='PASS' or m.get('blueprint_gate')!='PASS' or m.get('adversarial_second_pass')!='PASS': raise SystemExit('FINAL QA gate failure')
    ...
    for n in NEW_RANGE:
        ...
        if a.get('blind_audit',{}).get('selected_key')!=EXPECTED_KEYS[n] or a.get('second_possible_answer')!='PASS_NONE' or a.get('second_answer_attack',{}).get('result')!='PASS_NONE': raise SystemExit(f'Q{n}: second-answer/key failure')
        if a.get('defects')!=[] or a.get('suggested_changes')!=[] or len(a.get('scores',{}))!=10 or any(v!=10 for v in a.get('scores',{}).values()): raise SystemExit(f'Q{n}: defect/score failure')
        ...
        if n==1266 and (a.get('second_answer_attack',{}).get('strongest_alternative')!='E' or 'FLNC' not in a.get('second_answer_attack',{}).get('resolution','')): raise SystemExit('Q1266 repaired adversarial audit binding failure')
        audits[n]=(a,ap)
    return m,audits
...
def build_payload(template,d,n,audit,audit_path,manifest_hash):
    ...
    if bp.get('primary_system') not in {GI,CV} or bp.get('primary_competency') not in {DX,MK}: raise SystemExit(f'Q{n}: current blueprint failure')
...
def semantic_rescan(old_items,finals):
    ...
    for n in NEW_RANGE:
        ...
        for cid,ot,og,oc in seen:
            ...
            if jacc(ng,og)>=0.80: raise SystemExit(f'Q{n}: near duplicate {cid}')
...
def main():
    ...
    if pre.get('final_status')!='FINAL_10_10_PASS' or pre.get('item_count')!=EXPECTED_PRE_COUNT or pre.get('step2_final_review_count')!=EXPECTED_PRE_COUNT or pre.get('post_authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or pre.get('contiguous_q0001_q1260') is not True: raise SystemExit('pre-state Q1260 binding failure')
    ...
    if inc_bp!=Counter({GI:5,CV:5}) or inc_cp!=Counter({DX:5,MK:5}): raise SystemExit('new-block distribution failure')
    expected_bp=baseline_bp+inc_bp; expected_cp=baseline_cp+inc_cp
    template=old_payloads[0]; finals={}; reviews={}
    for n in NEW_RANGE:
        au,ap=audits[n]; c,r=build_payload(template,docs[n],n,au,ap,manifest_hash)
    ...
    for cid,pj,ps,ash in rows:
        ...
        if p['blueprint']['primary_system'] not in OFFICIAL_SYSTEMS or p['blueprint']['primary_competency'] not in OFFICIAL_COMPETENCIES: raise SystemExit(cid+' global metadata failure')
        n=qnum(cid)
        if n in NEW_RANGE:
            new_seen[n]=p
            if p['blueprint']['primary_system'] not in {GI,CV} or p['blueprint']['primary_competency'] not in {DX,MK}: raise SystemExit(cid+' current metadata failure')
            if p['blueprint']['official_outline_path']!=[p['blueprint']['primary_system']]: raise SystemExit(cid+' outline failure')
            if p['step2_final_audit']['final_10_10_gate']!='FINAL_10_10_PASS': raise SystemExit(cid+' FINAL gate failure')
    if set(new_seen)!=set(NEW_RANGE) or ''.join(new_seen[n]['item']['intended_key'] for n in NEW_RANGE)!='BECADCADEB': raise SystemExit('new block reread/key failure')
    ...
    new_disc=Counter();
    for p in new_seen.values(): new_disc.update(p['blueprint']['disciplines'])
    ...
    state={'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':EXPECTED_POST_COUNT,'authoritative_final_table':'step2_final_items','step2_final_review_count':EXPECTED_POST_COUNT,
      'new_block':{'range':'Q1261-Q1270','item_count':10,'clinical_audit_files_verified':10,'final_qa_manifest':str(MANIFEST.relative_to(REPO)),'final_qa_manifest_blob':EXPECTED_MANIFEST_BLOB,'final_qa_manifest_sha256':manifest_hash,'candidate_batch_blob':EXPECTED_BATCH_BLOB,'fresh_item_by_item_audit':True,'canonical_duplicate_rescan':True},
      'answer_position_new_block':{'balanced':dict(sorted(Counter(EXPECTED_KEYS.values()).items())),'sequence':'BECADCADEB'},'blueprint_counts':dict(post_bp),'competency_counts':dict(post_cp),'blueprint_increment_new_block':dict(inc_bp),'competency_increment_new_block':dict(inc_cp),'official_discipline_new_block_counts':{d:new_disc.get(d,0) for d in sorted(OFFICIAL_DISCIPLINES)},
      'official_system_labels_validated':True,'official_competency_labels_validated':True,'official_discipline_metadata_validated':True,'sqlite_integrity_check':'ok','duplicate_candidate_id_count':0,'payload_review_consistency':'PASS','reread_verified_count':EXPECTED_POST_COUNT,'new_block_reread_verified_count':10,'q1261_q1270_present_exactly_once':True,'contiguous_q0001_q1270':True,
      'pre_authoritative_db_blob':EXPECTED_PRE_DB_BLOB,'post_authoritative_db_blob':post_blob,'post_authoritative_db_sha256':post_sha,'wal_checkpoint_result':list(checkpoint) if checkpoint else None,'wal_sidecar_present_after_close':False,'shm_sidecar_present_after_close':False,'schema_tables_verified':['step2_final_items','step2_final_reviews','step2_finalization'],'finalized_at':now}
    ...
    assert s['item_count']==1270 and s['step2_final_review_count']==1270 and s['contiguous_q0001_q1270'] is True and s['q1261_q1270_present_exactly_once'] is True
    assert s['post_authoritative_db_blob']==gitblob(DB) and f['final_verdict']=='FINAL_10_10_PASS_NO_MATERIAL_DEFECT'
    print(json.dumps({'status':'SUCCESS','final_count':1270,'reviews':1270,'integrity':'ok','post_db_blob':post_blob,'state':str(STATE_POST.relative_to(REPO)),'final_audit':str(FINAL_AUDIT.relative_to(REPO))},sort_keys=True))
