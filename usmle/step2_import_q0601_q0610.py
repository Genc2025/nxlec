...
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0610.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0601_Q0610|2026-08-27'
ALLOWED_ROOTS=('medlineplus.gov','nih.gov','nlm.nih.gov','cdc.gov','fda.gov','hhs.gov','ahrq.gov','cms.gov','hrsa.gov','osha.gov','epa.gov','va.gov','federalregister.gov','ecfr.gov','congress.gov','cancer.gov','samhsa.gov','merckmanuals.com')
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {601+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f"S1-DIRECT-{x['num']:04d}-20260827T120000Z"
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,
        'step2_audit_id':'STEP2-FINAL-Q0601-Q0610-20260827',
...
def main():
    ...
    if len(old_items)!=600 or len(old_reviews)!=600:
        raise SystemExit(f'expected 600 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_nums!=set(range(1,601)):
        raise SystemExit('pre canonical Q0001-Q0600 coverage failure')
    ...
    seq=''.join(schedule[n] for n in range(601,611))
    ...
    try:
        ...
        con.execute(
            ...
            ('STEP2-FINAL-Q0001-Q0610-20260827',610,key_hash,aggregate_review_hash,FINAL_AT)
        )
        con.commit()
    except:
        con.rollback()
    ...
    if item_count!=610 or review_count!=610:
    ...
    if nums!=set(range(1,611)):
        raise SystemExit('post Q0001-Q0610 coverage failure')
    reread=0
    new_reread=0
    for cid,pj,ps,ash in con.execute("SELECT candidate_id,payload_json,payload_sha256,audit_sha256 FROM step2_final_items WHERE final_status='FINAL_10_10_PASS' ORDER BY candidate_id"):
        ...
        if 601<=n<=610:
            ...
            new_reread+=1
        reread+=1
    if reread!=610 or new_reread!=10:
    ...
    result={
        'audit_id':'STEP2-FINAL-Q0001-Q0610-20260827',
