...
import json,os,re,sqlite3,subprocess,time,urllib.request,urllib.error
from pathlib import Path
from urllib.parse import urlparse

ROOT=Path(__file__).resolve().parent
REPO=ROOT.parent
CAND=REPO/os.environ['CAND_PATH']
OUT=REPO/os.environ['OUT_PATH']
DB=ROOT/'data'/'usmle-step1.db'
CAND_BLOB=os.environ['CAND_BLOB']
...
def fetch_text(url):
 ...
 req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 USMLE-QA/4.3'})
 last=None
...
def evidence_key(ev,key,f):
 if isinstance(ev,dict):
  ...
  if strong!=[key]:
   f.append('evidence_derived_key')
  return strong[0] if len(strong)==1 else None
 if isinstance(ev,list):
  oe=[e for e in ev if isinstance(e.get('option'),str) and e.get('option') in 'ABCDE']
  em={e.get('option'):e for e in oe}
  if set(em)!=set('ABCDE') or len(oe)!=5:
   f.append('evidence_shape')
  strong=[L for L,e in em.items() if e.get('direct_or_inference') in {'direct','mixed'}]
  if strong!=[key]:
   f.append('evidence_derived_key')
  if any(em.get(L,{}).get('direct_or_inference')!='inference' for L in 'ABCDE' if L!=key):
   f.append('distractor_evidence_class')
  return strong[0] if len(strong)==1 else None
 f.append('evidence_shape')
 return None
...
def main():
 ...
 raw=CAND.read_text()
 assert 'ncjmm' not in raw.casefold()
 b=json.loads(raw)
 items=b['items']
 ...
 for cid,pj in rows:
  try:
   p=json.loads(pj)
   qi=p.get('item',{})
   t=' '.join([qi.get('vignette',''),qi.get('lead_in',''),*qi.get('options',{}).values(),qi.get('tested_construct','')])
  except Exception:
   t=pj
  canon.append((cid,t))
 ...
 for x in items:
  ...
  if list(opts)!=list('ABCDE') or len({norm(v) for v in opts.values()})!=5:
   f.append('options')
  if key not in 'ABCDE':
   f.append('key')
  ...
  if norm(ex.get('key_explanation',''))!=norm(de.get(key,'')) and kro<2:
   f.append('key_rationale_binding')

  if bp.get('official_outline_path')!=[bp.get('primary_system')] or not bp.get('primary_competency') or not bp.get('disciplines'):
   f.append('blueprint')
  ...
  if isinstance(ev,list):
   ...
   for e in ev:
    if e.get('source_ids') is not None and not set(e.get('source_ids',[])).issubset(ids):
     f.append('evidence_source_binding')
  ...
  if not author_ok or aq.get('unresolved_content_defects')!=[]:
   f.append('author_state')

  attack=aq.get('second_answer_attack',{})
  alt=attack.get('strongest_alternative') or attack.get('option')
  if alt not in opts or alt==key or len(attack.get('resolution','').strip())<30 or attack.get('status','PASS')!='PASS':
   f.append('second_answer_attack')
  ...
  for s in src:
   ...
   ok=True
   detail={}
   if not url.startswith('https://') or not (s.get('section_locator') or s.get('source_locator')) or not sid:
    ok=False
   try:
    if url not in cache:
     cache[url]=fetch_text(url)
    page=cache[url].casefold()
    tt=toks(s.get('title',''))
    sem=len(ctoks(ex.get('key_explanation',''))&ctoks(page))
    ov=len(tt&toks(page))
    if 'dailymed.nlm.nih.gov' in host:
     setid=str(s.get('setid','')).casefold()
     core={t for t in tt if t not in {'tablet','film','coated','injection','solution','kit','capsule','prescribing','information'}}
     co=len(core&toks(page))
     ok=ok and bool(setid) and setid in url.casefold() and co>=1 and sem>=2
     detail={'setid':setid,'title_core_overlap':co,'semantic_overlap':sem}
    elif 'pubmed.ncbi.nlm.nih.gov' in host:
     m=re.search(r'/([0-9]+)/?$',url)
     ok=ok and bool(m) and m.group(1) in page and ov>=2 and sem>=2
     detail={'pmid':m.group(1) if m else None,'title_overlap':ov,'semantic_overlap':sem}
    else:
     ok=ok and ov>=1 and sem>=1
     detail={'title_overlap':ov,'semantic_overlap':sem}
   except Exception as e:
    ok=False
    detail={'error':type(e).__name__}
   sr.append({'source_id':sid,'status':'PASS' if ok else 'BLOCKED',**detail})
   if not ok:
    f.append('live_source_binding')

  scored=sorted([(jac(item_text(x),t),cid) for cid,t in canon],reverse=True)
  mj=scored[0][0] if scored else 0
  if mj>=0.45:
   f.append('canonical_duplicate')
  top_canonical_text=next((t for cid,t in canon if scored and cid==scored[0][1]),'')

  reports.append({
   'q':q,
   'status':'PASS' if not f else 'BLOCKED',
   'evidence_derived_key':derived,
   'intended_key':key,
   'key_rationale_overlap':kro,
   'source_live_binding':'PASS' if 'live_source_binding' not in f else 'BLOCKED',
   'second_answer_attack':'PASS' if 'second_answer_attack' not in f else 'BLOCKED',
   'blueprint':'PASS' if 'blueprint' not in f else 'BLOCKED',
   'canonical_max_jaccard':round(mj,5),
   'canonical_top_match':scored[0][1] if scored else None,
   'canonical_top_text':top_canonical_text if mj>=0.45 else None,
   'source_reports':sr,
   'failures':sorted(set(f))
  })
 ...
 out={
  ...
  'failures':failures,
  'verdict':'ZERO_TRUST_PASS' if not failures else 'BLOCKED',
