...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0940.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0940.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='43422ba91de174f35a96861bb073c4e5235acc0e'
AUDIT_ID='STEP2-FINAL-Q0001-Q0940-20260901'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0931_Q0940|2026-09-01'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':121,
    'Blood, Lymphoreticular and Immune Systems':102,
    'Behavioral Health, Nervous Systems and Special Senses':109,
    'Musculoskeletal, Skin and Subcutaneous Tissue':89,
    'Cardiovascular System':87,
    'Gastrointestinal System':72,
    'Reproductive and Endocrine Systems':125,
...
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':633,
...
EXPECTED_NEW_DISCIPLINES={
    'Pathology':1,'Physiology':5,'Nutrition':0,'Gross Anatomy & Embryology':0,'Microbiology':5,
    'Pharmacology':9,'Behavioral Sciences':0,'Biochemistry':2,'Histology & Cell Biology':0,
    'Immunology':0,'Genetics':0
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {931+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260901T064500Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0931-Q0940-20260831',
...
def main():
    ...
    if len(old_items)!=930 or len(old_reviews)!=930:
        raise SystemExit(f'expected 930 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,931)):
    ...
    for n in range(931,941):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=930:
    ...
    seq=''.join(schedule[n] for n in range(931,941))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,940,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=940 or len(revs)!=940:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,941)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(931,941)):
        raise SystemExit('Q0931-Q0940 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=940:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':940,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':940,
        'new_block':{'range':'Q0931-Q0940','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'official_discipline_reference':{
            ...
            'note':'Q0931-Q0940 use explicit official Table 3 discipline tags. Legacy FINAL payloads remain immutable; cumulative Table 3 percentages are not fabricated from heterogeneous legacy labels.'
        },
        ...
        'payload_review_consistency':'PASS','reread_verified_count':940,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q0931_q0940_present_exactly_once':True,'contiguous_q0001_q0940':True,
