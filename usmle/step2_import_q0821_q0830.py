...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0830.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0830.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='6ac642ba334aca99f6afffdd80e829eb3ee0868e'
AUDIT_ID='STEP2-FINAL-Q0001-Q0830-20260831'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0821_Q0830|2026-08-31'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':103,
    'Blood, Lymphoreticular and Immune Systems':90,
    'Behavioral Health, Nervous Systems and Special Senses':96,
    'Musculoskeletal, Skin and Subcutaneous Tissue':82,
    'Cardiovascular System':75,
    'Gastrointestinal System':66,
    'Reproductive and Endocrine Systems':112,
    'Multisystem Processes and Disorders':83,
    'Biostatistics, Epidemiology and Population Health':45,
    'Social Sciences: Communication and Interpersonal Skills':61,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':544,
    'Patient Care: Diagnosis, including history and physical examination':178,
    'Practice-Based Learning and Improvement':46,
    'Communication and Interpersonal Skills':62,
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {821+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0821-Q0830-20260831',
...
def main():
    ...
    if len(old_items)!=820 or len(old_reviews)!=820:
        raise SystemExit(f'expected 820 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,821)):
    ...
    for n in range(821,831):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=820:
    ...
    seq=''.join(schedule[n] for n in range(821,831))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,830,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=830 or len(revs)!=830:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,831)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(821,831)):
        raise SystemExit('Q0821-Q0830 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=830:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':830,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':830,
        'new_block':{'range':'Q0821-Q0830','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':830,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q0821_q0830_present_exactly_once':True,'contiguous_q0001_q0830':True,
