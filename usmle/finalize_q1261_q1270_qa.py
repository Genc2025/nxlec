...
READONLY=ROOT/'audit'/'Q1261_Q1270_READONLY_AUDIT.json'
AUD=ROOT/'audit'
MANIFEST=AUD/'Q1261_Q1270_FINAL_QA_PASS.json'
CANON_BLOB='2afb11e1d3490cedc757ecfbe7e4515c6bbd527c'
CAND_BLOB='3061b7457d655f2923c442d2c59d09c23f2151a8'
...
def main():
    ...
    assert ro['canonical_db_blob']==CANON_BLOB and ro['canonical_count']==ro['canonical_review_count']==1260
    items={x['num']:x for x in batch['items']}; reports={x['q']:x for x in ro['item_reports']}
    assert set(items)==set(reports)==set(range(1261,1271))
    now=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
    files=[]
    for n in range(1261,1271):
        x=items[n]; r=reports[n]; key=x['item']['intended_key']
        assert r['status']=='PASS' and r['second_possible_answer']=='PASS_NONE' and r['adversarial_second_pass']=='PASS' and r['blind_audit']['status']=='PASS'
        assert r['failures']==[] and r['blind_audit']['selected_key']==key
        out={
          'audit_id':f'Q{n}-FINAL-10-10-20260906','item':f'Q{n}','status':'FINAL_10_10_PASS','audited_at':now,'auditor_model':'GPT-5.6 Sol',
          'authoritative_db':'usmle/data/usmle-step1.db','authoritative_db_blob':CANON_BLOB,'authoritative_db_final_count':1260,
          'exact_candidate_file':'usmle/batch_specs_1201_1300/09_q1261_q1270_author_20260906.json','exact_candidate_file_blob':CAND_BLOB,
          ...
          'official_system_gate':{'status':'PASS','canonical_label':x['blueprint']['primary_system']},
          'adversarial_second_pass':{'result':'PASS','note':'Fresh reread after source, locator/currentness, key, second-answer, canonical-neighbor, within-batch, blueprint, hidden-assumption, numerical and distractor attacks; no material defect remained.'},
          ...
          'blind_audit':{'selected_key':r['blind_audit']['selected_key'],'alternative_defensible_options':[],'missing_assumptions':[],'cueing_findings':[],'rationale':x['explanation']['key_explanation']+' Strongest alternative resolved: '+r['second_answer_attack']['resolution']}
        }
        p=AUD/f'Q{n}_FINAL_10_10_AUDIT.json'; p.write_text(json.dumps(out,indent=2,ensure_ascii=False)+'\n'); files.append((n,p,hobj(out)))
    manifest={
      'manifest_id':'Q1261-Q1270-FINAL-QA-PASS-Q1260-BOUND-20260906','status':'FINAL_QA_PASS','final_qa_verdict':'FINAL_QA_PASS_NO_MATERIAL_DEFECT','audited_at':now,'auditor_model':'GPT-5.6 Sol',
      'authoritative_db':'usmle/data/usmle-step1.db','authoritative_db_final_count':1260,'authoritative_db_blob':CANON_BLOB,
      'candidate_batch':'usmle/batch_specs_1201_1300/09_q1261_q1270_author_20260906.json','candidate_batch_blob':CAND_BLOB,'candidate_batch_object_sha256':hobj(batch),
      'readonly_audit_sha256':hobj(ro),'readonly_audit_verdict':'READONLY_QA_PASS','fresh_rerun_from_zero':True,'item_count':10,'item_range':'Q1261-Q1270',
      'answer_key_sequence':'BECADCADEB','answer_key_distribution':{'A':2,'B':2,'C':2,'D':2,'E':2},
      'system_distribution':{'Gastrointestinal System':5,'Cardiovascular System':5},
      ...
      'max_canonical_similarity':max(r['canonical_duplicate_gate']['max_jaccard'] for r in reports.values()),
