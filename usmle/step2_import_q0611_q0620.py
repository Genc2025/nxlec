...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0620.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0620.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0611_Q0620|2026-08-27'
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {611+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f"S1-DIRECT-{x['num']:04d}-20260827T154000Z"
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,
        'step2_audit_id':'STEP2-FINAL-Q0611-Q0620-20260827',
...
def main():
    ...
    if len(old_items)!=610 or len(old_reviews)!=610:
        raise SystemExit(f'expected 610 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_nums!=set(range(1,611)):
        raise SystemExit('pre canonical Q0001-Q0610 coverage failure')
    ...
    for n in range(611,621):
        ...
        finals[n]=c
        reviews[n]=r
    # Technical duplicate gates against authoritative 600 and within approved batch.
    ...
    seq=''.join(schedule[n] for n in range(611,621))
    ...
    try:
        ...
        con.execute(
            ...
            ('STEP2-FINAL-Q0001-Q0620-20260827',610,key_hash,aggregate_review_hash,FINAL_AT)
        )
        con.commit()
    except:
        con.rollback()
    ...
    if item_count!=620 or review_count!=620:
    ...
    if nums!=set(range(1,621)):
        raise SystemExit('post Q0001-Q0620 coverage failure')
    reread=0
    new_reread=0
    for cid,pj,ps,ash in con.execute("SELECT candidate_id,payload_json,payload_sha256,audit_sha256 FROM step2_final_items WHERE final_status='FINAL_10_10_PASS' ORDER BY candidate_id"):
        ...
        if 611<=n<=620:
            ...
            new_reread+=1
        reread+=1
    if reread!=620 or new_reread!=10:
    ...
    expected_blueprint={
        ...
        'Respiratory and Renal/Urinary Systems':81,
        'Blood, Lymphoreticular and Immune Systems':68,
        'Behavioral Health, Nervous Systems and Special Senses':74,
        'Musculoskeletal, Skin and Subcutaneous Tissue':62,
        'Cardiovascular System':56,
        'Gastrointestinal System':50,
        'Reproductive and Endocrine Systems':80,
        'Multisystem Processes and Disorders':62,
        'Biostatistics, Epidemiology and Population Health':31,
        'Social Sciences: Communication and Interpersonal Skills':43
    }
