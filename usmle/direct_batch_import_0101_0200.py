...
def build_pair(x):
 n=x["num"]; cid=f"S1-DIRECT-{n:04d}-20260827T021500Z"; key=x["key"]
...
def load_specs(spec_dir):
 ...
 if len(xs)!=100 or [x["num"] for x in xs]!=list(range(101,201)):
  raise SystemExit(f"spec set must contain exactly Q0101-Q0200; got {len(xs)} items")
 keys="ABCDE"*20
...
def main(spec_dir):
 ...
 if before!=100:raise SystemExit(f"expected authoritative count 100, found {before}")
 ...
 if after!=200 or after-before!=100:raise SystemExit(f"counter mismatch {before}->{after}")
 for p in pairs:
