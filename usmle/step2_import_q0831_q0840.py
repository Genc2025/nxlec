...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0840.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0840.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='c23b851ca561d5584d5ce0086bdba24679138c5c'
AUDIT_ID='STEP2-FINAL-Q0001-Q0840-20260831'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0831_Q0840|2026-08-31'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':104,
    'Blood, Lymphoreticular and Immune Systems':91,
    'Behavioral Health, Nervous Systems and Special Senses':97,
    'Musculoskeletal, Skin and Subcutaneous Tissue':83,
    'Cardiovascular System':76,
    'Gastrointestinal System':67,
    'Reproductive and Endocrine Systems':113,
    'Multisystem Processes and Disorders':84,
    'Biostatistics, Epidemiology and Population Health':46,
    'Social Sciences: Communication and Interpersonal Skills':62,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':550,
    'Patient Care: Diagnosis, including history and physical examination':180,
    'Practice-Based Learning and Improvement':47,
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {831+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0831-Q0840-20260831',
...
def main():
    ...
    if len(old_items)!=830 or len(old_reviews)!=830:
        raise SystemExit(f'expected 830 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,831)):
    ...
    for n in range(831,841):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=830:
    ...
    seq=''.join(schedule[n] for n in range(831,841))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,840,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=840 or len(revs)!=840:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,841)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(831,841)):
        raise SystemExit('Q0831-Q0840 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=840:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':840,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':840,
        'new_block':{'range':'Q0831-Q0840','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':840,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q0831_q0840_present_exactly_once':True,'contiguous_q0001_q0840':True,
