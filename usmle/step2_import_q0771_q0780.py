...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0780.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0780.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='1c621c9e756be9bc16a88d35dc543de5309e0d12'
AUDIT_ID='STEP2-FINAL-Q0001-Q0780-20260830'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0771_Q0780|2026-08-30'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':99,
    'Blood, Lymphoreticular and Immune Systems':84,
    'Behavioral Health, Nervous Systems and Special Senses':91,
    'Musculoskeletal, Skin and Subcutaneous Tissue':77,
    'Cardiovascular System':70,
    'Gastrointestinal System':62,
    'Reproductive and Endocrine Systems':107,
    'Multisystem Processes and Disorders':77,
    'Biostatistics, Epidemiology and Population Health':40,
    'Social Sciences: Communication and Interpersonal Skills':57,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':507,
    'Patient Care: Diagnosis, including history and physical examination':174,
    'Practice-Based Learning and Improvement':41,
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {771+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260830T180000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0771-Q0780-20260830',
...
def main():
    ...
    if len(old_items)!=770 or len(old_reviews)!=770:
        raise SystemExit(f'expected 770 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,771)):
    ...
    for n in range(771,781):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=770:
    ...
    seq=''.join(schedule[n] for n in range(771,781))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,780,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=780 or len(revs)!=780:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,781)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(771,781)):
        raise SystemExit('Q0771-Q0780 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=780:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':780,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':780,
        'new_block':{'range':'Q0771-Q0780','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':780,'new_block_reread_verified_count':10,
        'q0771_q0780_present_exactly_once':True,'contiguous_q0001_q0780':True,
