...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0670.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0670.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='01de9c00d4811e287aa9e75e1c0ac13e6f5232a9'
AUDIT_ID='STEP2-FINAL-Q0001-Q0670-20260829'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0661_Q0670|2026-08-29'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':86,
    'Blood, Lymphoreticular and Immune Systems':73,
    'Behavioral Health, Nervous Systems and Special Senses':79,
    'Musculoskeletal, Skin and Subcutaneous Tissue':66,
    'Cardiovascular System':59,
    'Gastrointestinal System':53,
    'Reproductive and Endocrine Systems':92,
    'Multisystem Processes and Disorders':66,
    'Biostatistics, Epidemiology and Population Health':33,
    'Social Sciences: Communication and Interpersonal Skills':49,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':434,
    'Patient Care: Diagnosis, including history and physical examination':152,
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {661+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0661-Q0670-20260829',
...
def main():
    ...
    if len(old_items)!=660 or len(old_reviews)!=660:
        raise SystemExit(f'expected 660 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,661)):
    ...
    for n in range(661,671):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=660:
    ...
    seq=''.join(schedule[n] for n in range(661,671))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,670,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=670 or len(revs)!=670:
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=670:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':670,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':670,
        'new_block':{'range':'Q0661-Q0670','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':670,'new_block_reread_verified_count':10,
        'q0661_q0670_present_exactly_once':True,'contiguous_q0001_q0670':True,
