...
SPEC_FILES = [
    (ROOT / 'batch_specs_1101_1200/12_q1191_q1195.json', 'cbcec34a0d1791f344a9226305680bc2eec3debe', 1191, 1195),
    (ROOT / 'batch_specs_1101_1200/13_q1196_q1200.json', '854078c41087450d3d05729c4f2fa430f1b5573e', 1196, 1200),
    (ROOT / 'batch_specs_1201_1300/01_q1201_q1205.json', '1e350647b0f68ff2e7a20cbfc903949307306179', 1201, 1205),
...
EXPECTED_PRE_DB_BLOB = '987c82084e1906854c03fe21e7dfc4f9d46be923'
...
EXPECTED_NEW_DISCIPLINES = {'Pathology': 4, 'Physiology': 16, 'Nutrition': 0, 'Gross Anatomy & Embryology': 0, 'Microbiology': 0, 'Pharmacology': 0, 'Behavioral Sciences': 5, 'Biochemistry': 10, 'Histology & Cell Biology': 3, 'Immunology': 0, 'Genetics': 19}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    c['step2_final_audit']={
        ...
        'preflight_spec_blobs':EXPECTED_SPEC_BLOBS,
        'preflight_db_blob':EXPECTED_PRE_DB_BLOB
    }
    review={
        'candidate_id':cid,
        'step2_audit_id':'STEP2-FINAL-Q1191-Q1215-20260904',
...
def main():
    ...
    if len(old_items)!=1190 or len(old_reviews)!=1190:
        raise SystemExit(f'expected 1190 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,1191)):
    ...
    if Counter(KEY_SCHEDULE.values())!=Counter({'A':5,'B':5,'C':5,'D':5,'E':5}):
        raise SystemExit('new key balance failure')
    seq=''.join(KEY_SCHEDULE[n] for n in NEW_RANGE)
    if len(seq)!=25 or len(set(seq[i:i+5] for i in range(0,25,5)))<5:
    ...
    for n in NEW_RANGE:
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=1190:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',
                    (AUDIT_ID,1215,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback()
        raise
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=1215:
    ...
    result={
        ...
        'step2_final_review_count':1215,
        'new_block':{
            'range':'Q1191-Q1215',
            'item_count':25,
            'clinical_audit_files_verified':25,
        ...
        'reread_verified_count':1215,
        'new_block_reread_verified_count':25,
        ...
        'q1191_q1215_present_exactly_once':True,
        'contiguous_q0001_q1215':True,
