...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0720.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0720.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='3a44ba289ae5b17c9bb85355709da59bfe16b423'
AUDIT_ID='STEP2-FINAL-Q0001-Q0720-20260829'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0711_Q0720|2026-08-29'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Respiratory and Renal/Urinary Systems':92,
    'Blood, Lymphoreticular and Immune Systems':78,
    'Behavioral Health, Nervous Systems and Special Senses':85,
    'Musculoskeletal, Skin and Subcutaneous Tissue':71,
    'Cardiovascular System':64,
    'Gastrointestinal System':57,
    'Reproductive and Endocrine Systems':99,
    'Multisystem Processes and Disorders':71,
    'Biostatistics, Epidemiology and Population Health':36,
    'Social Sciences: Communication and Interpersonal Skills':53,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':467,
    'Patient Care: Diagnosis, including history and physical examination':162,
    'Practice-Based Learning and Improvement':37,
    'Communication and Interpersonal Skills':54,
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {711+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260829T094500Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0711-Q0720-20260829',
...
def main():
    ...
    if len(old_items)!=710 or len(old_reviews)!=710:
        raise SystemExit(f'expected 710 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,711)):
    ...
    for n in range(711,721):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=710:
    ...
    seq=''.join(schedule[n] for n in range(711,721))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,720,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=720 or len(revs)!=720:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,721)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(711,721)):
        raise SystemExit('Q0711-Q0720 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=720:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':720,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':720,
        'new_block':{'range':'Q0711-Q0720','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':720,'new_block_reread_verified_count':10,
        'q0711_q0720_present_exactly_once':True,'contiguous_q0001_q0720':True,
