...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q1000.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q1000.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='d48fe12bb4b8cc2e7a0fbfcb58003b49df9662ac'
AUDIT_ID='STEP2-FINAL-Q0001-Q1000-20260902'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0991_Q1000|2026-09-02'
SYSTEM_ALIASES={
...
EXPECTED_NEW_DISCIPLINES={
    "Pathology":2,"Physiology":8,"Nutrition":0,"Gross Anatomy & Embryology":0,"Microbiology":0,"Pharmacology":1,"Behavioral Sciences":1,"Biochemistry":7,"Histology & Cell Biology":1,"Immunology":0,"Genetics":4
}
...
def load_audits():
    ...
    for n in range(991,1001):
        ...
        if a.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(a.get('authoritative_db_final_count',-1))!=990:
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {991+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260902T004000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0991-Q1000-20260902',
...
def main():
    ...
    current_spec_blob=subprocess.check_output(['git','-C',str(REPO),'hash-object','usmle/batch_specs_0901_1000/10_q0991_q1000.json'],text=True).strip()
    ...
    if len(old_items)!=990 or len(old_reviews)!=990:
        raise SystemExit(f'expected 990 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,1001)):
    ...
    for n in range(991,1001):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=990:
    ...
    seq=''.join(schedule[n] for n in range(991,1001))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,1000,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=1000 or len(revs)!=1000:
    ...
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(991,1001)):
        raise SystemExit('Q0991-Q1000 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=1000:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':1000,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':1000,
        'new_block':{'range':'Q0991-Q1000','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'official_discipline_reference':{
            ...
            'note':'Q0991-Q1000 use explicit official Table 3 discipline tags. Legacy FINAL payloads remain immutable; cumulative Table 3 percentages are not fabricated from heterogeneous legacy labels.'
        },
        ...
        'payload_review_consistency':'PASS','reread_verified_count':1000,'new_block_reread_verified_count':10,'expert_review_layer_required':True,'key_integrity_gate_required':True,'canonical_main_construct_overlap_required':True,
        'q0991_q1000_present_exactly_once':True,'contiguous_q0001_q1000':True,
