...
SPEC = ROOT / 'batch_specs_1101_1200' / '09_q1176_q1180.json'
AUDIT_DIR = ROOT / 'audit'
PREFLIGHT = AUDIT_DIR / 'PREFLIGHT_Q1176_Q1180.json'
FINAL_AUDIT = AUDIT_DIR / 'STEP2_FINAL_10_10_Q0001_Q1180.json'
FINAL_STATE = ROOT / 'state' / 'step2_final_q0001_q1180.json'
FINAL_AT = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')

EXPECTED_PRE_DB_BLOB = 'c42033ba0c8de7bdb6aae7d9c978dcef24123a64'
EXPECTED_SPEC_BLOB = 'f0b337fae4b13a0a75a994b67010144e37c4f127'
AUDIT_ID = 'STEP2-FINAL-Q0001-Q1180-20260904'
NEW_RANGE = range(1176,1181)
KEY_SCHEDULE = {1176:'B',1177:'D',1178:'A',1179:'C',1180:'E'}
...
def load_preflight():
    ...
    if p.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(p.get('authoritative_final_count',-1))!=1175:
...
def load_audits():
    ...
    for n in NEW_RANGE:
        ...
        if a.get('authoritative_db_blob')!=EXPECTED_PRE_DB_BLOB or int(a.get('authoritative_db_final_count',-1))!=1175:
...
def build_payload(template,x,key,audit,audit_path):
    ...
    cid=f'S1-DIRECT-{n:04d}-20260904T043000Z'
    c['candidate_id']=cid
    c['blueprint']={
    ...
    review={
        'candidate_id':cid,
        'step2_audit_id':'STEP2-FINAL-Q1176-Q1180-20260904',
...
def main():
    ...
    if len(old_items)!=1175 or len(old_reviews)!=1175:
        raise SystemExit(f'expected 1175 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,1176)):
    ...
    for n in NEW_RANGE:
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=1175:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',
                    (AUDIT_ID,1180,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback()
        raise
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=1180:
    ...
    result={
        ...
        'q1176_q1180_present_exactly_once':True,
        'contiguous_q0001_q1180':True,
