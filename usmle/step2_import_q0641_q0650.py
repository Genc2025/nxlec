...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0650.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0650.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='fd94212d6c2c20abddb8ebfdd4d7cb89beeb3bdd'
AUDIT_ID='STEP2-FINAL-Q0001-Q0650-20260828'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0641_Q0650|2026-08-28'
SYSTEM_ALIASES={
...
EXPECTED_BLUEPRINT={
    ...
    'Behavioral Health, Nervous Systems and Special Senses':77,
    'Musculoskeletal, Skin and Subcutaneous Tissue':64,
    'Cardiovascular System':57,
    'Gastrointestinal System':51,
    'Reproductive and Endocrine Systems':90,
    'Multisystem Processes and Disorders':64,
    'Biostatistics, Epidemiology and Population Health':32,
    'Social Sciences: Communication and Interpersonal Skills':48,
}
EXPECTED_COMPETENCIES={
    'Medical Knowledge: Applying Foundational Science Concepts':431,
    'Patient Care: Diagnosis, including history and physical examination':140,
    'Practice-Based Learning and Improvement':32,
    'Communication and Interpersonal Skills':47,
}
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {641+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0641-Q0650-20260828',
...
def main():
    ...
    if len(old_items)!=640 or len(old_reviews)!=640:
        raise SystemExit(f'expected 640 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,641)):
    ...
    for n in range(641,651):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=640:
    ...
    seq=''.join(schedule[n] for n in range(641,651))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,650,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=650 or len(revs)!=650:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,651)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(641,651)):
        raise SystemExit('Q0641-Q0650 exact-once failure')
    ...
    result={
        ...
        'new_block':{'range':'Q0641-Q0650','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':650,'new_block_reread_verified_count':10,
        'q0641_q0650_present_exactly_once':True,'contiguous_q0001_q0650':True,
