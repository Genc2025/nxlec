...
import q1636_item,q1637_item,q1638_item,q1639_item,q1640_item

OUT=Path('usmle/batch_specs_1601_1700/08_q1636_q1640_author_20260919.json')
DB_BLOB=os.environ.get('DB_BLOB','')
CANONICAL_COUNT=int(os.environ.get('CANONICAL_COUNT','1635'))
TODAY='2026-09-19'
...
def main():
 ...
 assert CANONICAL_COUNT==1635
 items=[m.build(make_general) for m in (q1636_item,q1637_item,q1638_item,q1639_item,q1640_item)]
 b={'batch_id':'Q1636-Q1640-20260919','created_at':TODAY,'status':'AUTHOR_QA_PASS_PENDING_TECHNICAL_FREEZE_AND_INDEPENDENT_AUDIT','production_import_ready':False,'candidate_count':5,'canonical_count_before':CANONICAL_COUNT,'canonical_count_after':CANONICAL_COUNT,'canonical_db_blob':DB_BLOB,'answer_key_sequence':'ABCDE','answer_key_distribution':{L:1 for L in 'ABCDE'},'construct_selection':'Prospective whole-bank screening against authoritative Q0001-Q1635 production plus source-first adversarial authoring; FLVCR1 was excluded because newer literature creates a material transporter-function ambiguity.','technical_integrity':{'source_hashes_fabricated':False,'source_hashes_complete':False,'independent_audit_complete':False,'trusted_importer_complete':False},'items':items}
 OUT.parent.mkdir(exist_ok=True); OUT.write_text(json.dumps(b,indent=2,ensure_ascii=False)+'\n')
 assert [x['num'] for x in items]==list(range(1636,1641))
