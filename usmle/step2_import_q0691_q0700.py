...
FINAL_AUDIT=AUDIT_DIR/'STEP2_FINAL_10_10_Q0001_Q0700.json'
FINAL_STATE=ROOT/'state'/'step2_final_q0001_q0700.json'
FINAL_AT=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z')
EXPECTED_PRE_DB_BLOB='9fb83fdc08d56fc72df00ad0f5fe3d94940e1abd'
AUDIT_ID='STEP2-FINAL-Q0001-Q0700-20260829'
SEED_TEXT='USMLE_STEP1_APPROVED_BATCH_Q0691_Q0700|2026-08-29'
SYSTEM_ALIASES={
...
def new_schedule():
    ...
    for _ in range(10000):
        ...
        if seq[5:] in ('ABCDE','BCDEA','CDEAB','DEABC','EABCD'):
            continue
        return {691+i:base[i] for i in range(10)}
...
def build_payload(template,x,key,audit,audit_path):
    ...
    review={
        'candidate_id':cid,'step2_audit_id':'STEP2-FINAL-Q0691-Q0700-20260829',
...
def main():
    ...
    if len(old_items)!=690 or len(old_reviews)!=690:
        raise SystemExit(f'expected 690 authoritative finals/reviews, got {len(old_items)}/{len(old_reviews)}')
    ...
    if old_ids!=review_ids or {qnum(cid) for cid in old_ids}!=set(range(1,701)):
    ...
    for n in range(691,701):
        ...
        if c['candidate_id'] in old_ids or qnum(c['candidate_id'])<=690:
    ...
    seq=''.join(schedule[n] for n in range(691,701))
    if max(len(m.group(0)) for m in re.finditer(r'(.)\1*',seq))>2:
    ...
    try:
        ...
        con.execute('UPDATE step2_finalization SET audit_id=?,item_count=?,key_schedule_sha256=?,aggregate_review_sha256=?,finalized_at=? WHERE id=1',(AUDIT_ID,700,key_hash,aggregate_review_hash,FINAL_AT))
        con.commit()
    except:
        con.rollback(); raise
    if con.execute('PRAGMA integrity_check').fetchone()[0]!='ok':
    ...
    if len(items)!=700 or len(revs)!=700:
    ...
    if ids!=rids or {qnum(cid) for cid in ids}!=set(range(1,701)):
        raise SystemExit('post item/review/contiguity failure')
    if any(sum(1 for cid in ids if qnum(cid)==n)!=1 for n in range(691,701)):
        raise SystemExit('Q0691-Q0700 exact-once failure')
    ...
    if not fin or fin[0]!=AUDIT_ID or fin[1]!=700:
    ...
    result={
        'audit_id':AUDIT_ID,'final_status':'FINAL_10_10_PASS','item_count':700,
        'authoritative_final_table':'step2_final_items','step2_final_review_count':700,
        'new_block':{'range':'Q0691-Q0700','item_count':10,'clinical_audit_files_verified':10,'fresh_item_by_item_audit':True},
        ...
        'payload_review_consistency':'PASS','reread_verified_count':700,'new_block_reread_verified_count':10,
        'q0691_q0700_present_exactly_once':True,'contiguous_q0001_q0700':True,
